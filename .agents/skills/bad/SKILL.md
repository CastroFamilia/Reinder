...
13. **Worktree lifecycle** — worktrees lock their branches; `gh pr merge --delete-branch` and `git branch -D` will fail while a worktree exists. **Never use `--delete-branch` in merge commands.** Worktree removal and branch cleanup are handled exclusively by the Phase 3 cleanup subagent, which runs after all merges complete. If the user requests manual merges outside Phase 3, always remove worktrees first (`git worktree remove --force <path>`), then merge without `--delete-branch`, then delete branches manually.
