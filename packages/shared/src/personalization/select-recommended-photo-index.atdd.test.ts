...
describe("Story 10.3 — AC1/AC8: selectRecommendedPhotoIndex()", () => {
  ...
  test("[P0] T10.3-02: skips out-of-range indices and selects first valid", () => {
  ...
  test("[P0] T10.3-03: returns 0 (fallback) when preferredPhotoIndices is empty", () => {
  ...
  test("[P0] T10.3-04: returns null when imageCount is 0", () => {
  ...
  test("[P0] T10.3-05: returns null when preferredIndices is empty and imageCount is 0", () => {
  ...
  test("[P0] T10.3-06: returns 0 (fallback) when all indices >= imageCount", () => {
  ...
  test("[P1] T10.3-07: listing with 1 photo always returns 0", () => {
    // AC8: Listing con 1 sola foto → siempre 0
    const result = selectRecommendedPhotoIndex([0, 2, 4], 1);
    expect(result).toBe(0);
  });

  test("[P1] T10.3-08: listing with 1 photo and empty prefs returns 0", () => {
    const result = selectRecommendedPhotoIndex([], 1);
    expect(result).toBe(0);
  });

  test("[P1] T10.3-09: selects index 0 when it is the only valid one", () => {
    const result = selectRecommendedPhotoIndex([5, 0, 3], 1);
    expect(result).toBe(0);
  });

  test("[P2] T10.3-10: handles large preferredPhotoIndices arrays", () => {
  ...
  test("[P2] T10.3-11: handles preferredPhotoIndices with duplicates", () => {
